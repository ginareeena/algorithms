 ...
 *
 * Implement your function inside this test file. This test file has 4 tests
 ...
 **/
  ...
  * and does not conflict with teacher's schedule.
  *
 ...
 function suggestSchedule(parentRequest, teacher) {
   // TODO: Implement this function
    ...
    for(let i=0;i<days.length;i++){
      let currDay= days[i]
      // array of times for that day
      let teacherDay= availability[currDay]
      // array of times for request day
      let parentDay= parentRequest[currDay]
      
      if(teacherDay && parentDay){
        
        for(let j=0;j<parentDay.length;j++){
          // if the teacher times for that day includes this request time
        if(teacherDay.includes(parentDay[j])){
          let result= 
          {
           start_time: twoWeeksLater(currDay, parentDay[j]),
           end_time: twoWeeksLater(currDay,parentDay[j]+1)
          }
          
          // an array of sections
          let bookedTimes= getTeacherSchedule(teacher)
          ...
          // let resultTime= result.start_time.getTime()
          // let teacherTime= bookedTimes[k].start
          
          if(result.start_time.getTime() !== bookedTimes[k].start_time.getTime()){
